...
class EventValidatorTest {
    ...
    @Test
    fun `test succesfull validation accessing S3`() {
        val payload = """
                        "users": [
                            {
                                "name": "Bruno",
                        ...
                        "l": "Thiago"
                """.trimIndent()
        val s3SchemaRepository = S3SchemaRepository("event-schemas-staging", Regions.SA_EAST_1)
        val schemaDataRepository = SchemaDataRepository(s3SchemaRepository)
        val cachedSchemaRepository = CachedSchemaRepository(schemaDataRepository, 10, 10)
        val eventValidator = EventValidator(cachedSchemaRepository)
        eventValidator.validate(newEvent("event_test", 1, payload))
    }
